...
function Vote() {
  ...
  const handleVote = (poolId: string) => {
    ...
    if (parseFloat(governance.votingPower) === 0) {
      alert(
        "You need ICE tokens to vote. Stake AQUA to earn ICE tokens - rewards increase over time."
      );
      return;
    }
    ...
    console.log(
      `Voting for pool ${poolId} with ${governance.votingPower} voting power`
    );
  };
                  ...
                  <div className="flex items-center space-x-3 mb-2">
                    ...
                    <div className="flex space-x-2">
                      {pool.assets.map((asset) => (
                        <span
                          key={asset}
                          className="bg-[#2A2E3E] px-2 py-1 rounded-[6px] text-xs text-[#B1B3B8]"
                        >
                          {asset}
                        </span>
                      ))}
        ...
        {parseFloat(governance.votingPower) === 0 && user.userWalletAddress && (
          <div className="mt-8 bg-[#2A2E3E] p-6 rounded-[12px] border border-[#3C404D]">
            ...
            <p className="text-[#B1B3B8] text-sm mb-3">
              To participate in governance, you need ICE tokens. Here's how to
              get them:
            </p>
            <ol className="text-[#B1B3B8] text-sm space-y-1 ml-4">
              <li>1. Stake AQUA tokens to participate in governance</li>
              <li>
                2. Earn ICE tokens and rewards that increase the longer you
                stake
              </li>
              <li>
                3. Use ICE tokens to vote and direct Protocol Owned Liquidity
                rewards
              </li>
              <li>
                4. Unstake anytime - rewards are calculated based on time staked
              </li>
            </ol>
          </div>
        )}
          ...
          <div className="mt-8 bg-[#1A1E2E] p-6 rounded-[12px]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                ...
                <div className="text-white font-medium">
                  {parseFloat(governance.globalStats.totalVotingPower).toFixed(
                    2
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="text-[#00CC99] font-medium">
                  {parseFloat(
                    governance.globalStats.polVotingAllocation
                  ).toFixed(2)}
                </div>
              </div>
            </div>
