...
export class SorobanStakingService {
  ...
  async stakeAqua(
    ...
    try {
      console.log('🔒 [SorobanStaking] Staking AQUA:', { userAddress, amount, durationDays });
  ...
  async unstakeAqua(
    ...
    try {
      console.log('🔓 [SorobanStaking] Unstaking AQUA:', { userAddress, lockId, amount });
  ...
  async restakeBlub(
    ...
    try {
      console.log('🔄 [SorobanStaking] Restaking BLUB:', { userAddress, amount });

      // Call backend to handle the restaking
      const response = await axios.post(`${this.backendApi}/soroban/restake-blub`, {
        userAddress,
        amount,
        signedTxXdr,
      });
   ...
   */
  async getUserStakingInfo(userAddress: string): Promise<ContractCallResult<StakingInfo>> {
    try {
      console.log('📊 [SorobanStaking] Getting user staking info:', userAddress);

      // Try contract first
      const contractResult = await this.sorobanService.simulateContract<StakingInfo>(
        'staking',
      ...
      // Fallback to backend
      const response = await axios.get(`${this.backendApi}/soroban/user/${userAddress}/staking-info`);
   ...
   */
  async getUserLocks(userAddress: string): Promise<ContractCallResult<LockInfo[]>> {
    try {
      console.log('🔐 [SorobanStaking] Getting user locks:', userAddress);

      // Try contract first
      const contractResult = await this.sorobanService.simulateContract<LockInfo[]>(
        'staking',
        'get_user_locks',
        [userAddress],
      );
      ...
      // Fallback to backend
      const response = await axios.get(`${this.backendApi}/soroban/user/${userAddress}/locks`);
  ...
  async getProtocolOwnedLiquidity(): Promise<ContractCallResult<PolInfo>> {
    try {
      ...
      // Try contract first
      const contractResult = await this.sorobanService.simulateContract<PolInfo>(
        'staking',
  ...
  calculateRewardMultiplier(durationDays: number): number {
    ...
    return baseMultiplier + ((maxMultiplier - baseMultiplier) * durationMultiplier);
  }
  ...
  async getTotalStakingStats(): Promise<ContractCallResult<any>> {
    try {
      ...
      const response = await axios.get(`${this.backendApi}/soroban/staking-stats`);
  ...
  async healthCheck(): Promise<{ success: boolean; services: any }> {
    try {
      const [contractHealth, backendHealth] = await Promise.all([
        ...
        axios.get(`${this.backendApi}/health`).then(r => ({ success: true })).catch(() => ({ success: false })),
      ]);
